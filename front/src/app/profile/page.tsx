   ...
   const handleLogout = () => {
     ...
     useEffect(() => {
        const getProfile = async () => {
            try {
            const res = await api.get<AuthRes>('auth/profile');
            setUser(res.data);
                console.log(res);
            }
            catch {
                 console.error("Сначало войдите в аккаунт.");
            } 
        }
        getProfile();
}, [])
    ...
    <div className="bg-white p-8 rounded-lg shadow-md border border-gray-200 text-center w-64">
      ...
      <h2 className="text-gray-400 text-xs uppercase font-bold mb-2">Пользователь</h2>
